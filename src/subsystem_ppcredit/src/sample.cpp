...
void Sample::orderGoalsUsingTSP()
{
    ...
    double graph[V][V]; // 2D array to store distances
    ...
    for (i = 0; i < V; i++)
    {
        for (j = 0; j < V; j++)
        {
            if (i == j) {
                ...
                graph[i][j] = totalDistanceFromPlan(unordered_goals_[i].pose.position, unordered_goals_[j].pose.position);
            };
        }
    }
    ...
    int s = 0;
    double minimum_path = findMinPath(graph, s);
    ...
    // PRINT VECTOR
    std::cout << "Saved permuation with minimum weighted path: " << std::endl;
    for (int num : saved_perm_) {
        std::cout << num << " ";
    }
    std::cout << std::endl;
...
// FIX
double Sample::findMinPath(double graph[][NumOfDest], int s)
{
    int V = NumOfDest;
    ...
    do {
        ...
        std::cout << "Current_pathweight: " << current_pathweight << std::endl;
    ...
    std::cout << "MINIMUM CALCULATED PATH: " << min_path << std::endl;
...
{
    std::this_thread::sleep_for(std::chrono::milliseconds(10)); //500
    uint32_t idx;
...
{
    std::this_thread::sleep_for(std::chrono::milliseconds(5)); //10
